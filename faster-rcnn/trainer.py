...
class FasterRCNNTrainer(nn.Module):
    ...
    def forward(self, imgs, bboxes, labels, scale):
        ...
        for i in range(n):
            ...
            # -------------------------------------------------- #
            """
            在计算网络的回归损失时，只有正样本才有对应的回归目标，因此只有正样本需要参与计算。
            """
            rpn_loc_loss = _fast_rcnn_loc_loss(rpn_loc, gt_rpn_loc, gt_rpn_label, self.rpn_sigma)
            # ignore_index参数用于指定在损失计算过程中应该忽略的目标值
            rpn_cls_loss = F.cross_entropy(rpn_score, gt_rpn_label, ignore_index=-1) # rpn_score:一张图片上所有anchor的标签，有0，1，-1
